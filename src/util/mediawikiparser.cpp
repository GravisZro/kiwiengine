#include "mediawikiparser.h"

namespace CoreParserFunctions
{
void registerObj(void*){}
}

namespace CoreTagHooks
{
void registerObj(void*){}
}

void wfRunHooks(QString, void*){}
bool extension_loaded(QString) { return true; }
bool defined(QString) { return true; }


MediaWikiParser::MediaWikiParser(const QMap<QVariant, QVariant>& conf)
  : mConf(conf),
    mMarkerIndex(0),
    mFirstCall(true)
{
  mUrlProtocols = ::wfUrlProtocols();
  mExtLinkBracketedRegex = QString("\\[((%1)[^][<>\"\\x00-\\x20\\x7F]+) *([^\\]\\x00-\\x08\\x0a-\\x1F]*?)\\]").arg(mUrlProtocols);

  if(mConf.contains("preprocessorClass"))
    mPreprocessorClass = mConf["preprocessorClass"];
  else if(::defined("MW_COMPILED"))
    mPreprocessorClass = "Preprocessor_Hash"; // Preprocessor_Hash is much faster than Preprocessor_DOM in compiled mode
  else if(::extension_loaded("domxml"))
    mPreprocessorClass = "Preprocessor_Hash";
  else if(::extension_loaded("dom"))
    mPreprocessorClass = "Preprocessor_DOM";
  else
    mPreprocessorClass = "Preprocessor_Hash";

//  wfDebug( __CLASS__ . ": using preprocessor: {mPreprocessorClass}\n" );
}

MediaWikiParser::~MediaWikiParser(void)
{
}

void MediaWikiParser::firstCallInit(void)
{
  if (!mFirstCall )
    return;
  mFirstCall = false;

  CoreParserFunctions::registerObj(this);
  CoreTagHooks::registerObj(this);
  initialiseVariables();

  ::wfRunHooks("ParserFirstCallInit", this);
}

void MediaWikiParser::clearState(void)
{
  if ( mFirstCall )
    firstCallInit();

	mOutput = new ParserOutput;
	mOptions->registerWatcher( array( mOutput, 'recordOption' ) );
	mAutonumber = 0;
	mLastSection = "";
	mDTopen = false;
	mIncludeCount = array();
	mArgStack = false;
	mInPre = false;
	mLinkHolders = new LinkHolderArray( this );
	mLinkID = 0;
	mRevisionObject = mRevisionTimestamp =
		mRevisionId = mRevisionUser = nullptr;
	mVarCache.clear();
	mUser = nullptr;
	 ...
	 */
	// mUniqPrefix = "\x07UNIQ" . Parser::getRandomString();
	// Changed to \x7f to allow XML double-parsing -- TS
	mUniqPrefix = "\x7fUNIQ" . self::getRandomString();
	mStripState = new StripState( mUniqPrefix );


	// Clear these on every parse, bug 4549
	mTplExpandCache.clear();
	mTplRedirCache.clear();
	mTplDomCache.clear();

	mShowToc = true;
	mForceTocPosition = false;
	mIncludeSizes["post-expand"] = 0;
	mIncludeSizes["arg"] = 0;
	...
	mDefaultSort = false;
	mHeadings.clear();
	mDoubleUnderscores.clear();
	mExpensiveFunctionCount = 0;

	if(mPreprocessor->isSet() && mPreprocessor->parser != this)
...
QString MediaWikiParser::cleanSigInSig(QString text)
{
  text.replace(QRegExp( "/~{3,5}/"), "");
  return text;
}

QString MediaWikiParser::getSection(QString text, QString section, QString deftext)
{
  return extractSections(text, section, "get", deftext);
}

QString MediaWikiParser::replaceSection(QString oldtext, QString section, QString text )
{
  return extractSections(oldtext, section, "replace", text);
}
 ...
 */
ParserOutput MediaWikiParser::parse(QString text, Title title, ParserOptions options, bool linestart, bool clearself::OT_State, Nullable<int> revid)
{
	/**
	...
	startParse( title, options, HTML, clearState );

	oldRevisionId = mRevisionId;
	oldRevisionObject = mRevisionObject;
	oldRevisionTimestamp = mRevisionTimestamp;
	oldRevisionUser = mRevisionUser;

	if ( revid !== nullptr) {
		mRevisionId = revid;
		mRevisionObject = nullptr;
		mRevisionTimestamp = nullptr;
		mRevisionUser = nullptr;
	}

	wfRunHooks( 'ParserBeforeStrip', array( &this, &text, &mStripState ) );
	// No more strip!
	wfRunHooks( 'ParserAfterStrip', array( &this, &text, &mStripState ) );
	...
	text = doBlockLevels( text, linestart );
	...
	if ( !( wgDisableLangConversion
			|| isset( mDoubleUnderscores['nocontentconvert'] )
			|| mTitle->isConversionTable() ) ) {
	...
	if ( !( wgDisableLangConversion
			...
			|| isset( mDoubleUnderscores['nocontentconvert'] )
			|| isset( mDoubleUnderscores['notitleconvert'] )
			|| mOutput->getDisplayTitle() !== false ) )
	{
		convruletitle = wgContLang->getConvRuleTitle();
		if ( convruletitle ) {
			mOutput->setTitleText( convruletitle );
		} else {
			titleText = wgContLang->convertTitle( title );
			mOutput->setTitleText( titleText );
		}
	}
	...
	text = replaceTransparentTags( text );
	text = mStripState->unstripGeneral( text );

	text = Sanitizer::normalizeCharReferences( text );

	if ( ( wgUseTidy && mOptions->getTidy() ) || wgAlwaysUseTidy ) {
	...
	if ( mExpensiveFunctionCount > wgExpensiveParserFunctionLimit ) {
		limitationWarn( 'expensive-parserfunction', mExpensiveFunctionCount, wgExpensiveParserFunctionLimit );
	}

	wfRunHooks( 'ParserAfterTidy', array( &this, &text ) );

	// Information on include size limits, for the benefit of users who try to skirt them
	if ( mOptions->getEnableLimitReport() ) {
		max = mOptions->getMaxIncludeSize();
		PFreport = "Expensive parser function count: {mExpensiveFunctionCount}/wgExpensiveParserFunctionLimit\n";
		limitReport =
			"NewPP limit report\n" .
			"Preprocessor node count: {mPPNodeCount}/{mOptions->getMaxPPNodeCount()}\n" .
			"Post-expand include size: {mIncludeSizes['post-expand']}/max bytes\n" .
			"Template argument size: {mIncludeSizes['arg']}/max bytes\n".
			PFreport;
	...
	mRevisionObject = oldRevisionObject;
	mRevisionTimestamp = oldRevisionTimestamp;
	mRevisionUser = oldRevisionUser;
...
function recursiveTagParse( text, frame=false ) {
	...
	wfRunHooks( 'ParserBeforeStrip', array( &this, &text, &mStripState ) );
	wfRunHooks( 'ParserAfterStrip', array( &this, &text, &mStripState ) );
	text = internalParse( text, false, frame );
	wfProfileOut( __METHOD__ );
	return text;
}
 ...
 */
function preprocess( text, Title title, ParserOptions options, revid = nullptr) {
	wfProfileIn( __METHOD__ );
	startParse( title, options, PREPROCESS, true );
	if ( revid !== nullptr) {
		mRevisionId = revid;
	}
	wfRunHooks( 'ParserBeforeStrip', array( &this, &text, &mStripState ) );
...
public function getPreloadText( text, Title title, ParserOptions options ) {
	...
	startParse( title, options, PLAIN, true );

	flags = PPFrame::NO_ARGS | PPFrame::NO_TEMPLATES;
	dom = preprocessToDom( text, self::PTD_FOR_INCLUSION );
	text = getPreprocessor()->newFrame()->expand( dom, flags );
	text = mStripState->unstripBoth( text );
	return text;
}
 ...
 *
 * @param user Mixed: User object or nullptr(to reset)
 */
 ...
 *
 * @param x New Title object or nullptrto just get the current one
 * @return Title object
 */
function Title( x = nullptr) {
 ...
 *
 * @param x New value or nullptrto just get the current one
 * @return Integer
 */
function OutputType( x = nullptr) {
...
function getFunctionLang() {
	target = mOptions->getTargetLanguage();
	if ( target !== nullptr) {
		return target;
	} elseif( mOptions->getInterfaceMessage() ) {
		global wgLang;
		return wgLang;
	} elseif( is_null( mTitle ) ) {
		throw new MWException( __METHOD__.': mTitle is null' );
	}
	return mTitle->getPageLanguage();
}

/**
 * Get a User object either from mUser, if set, or from the
...
function getPreprocessor() {
	if ( !isset( mPreprocessor ) ) {
		class = mPreprocessorClass;
		mPreprocessor = new class( this );
	}
...
public static function extractTagsAndParams( elements, text, &matches, uniq_prefix = '' ) {
	...
	while ( text != '' ) {
		...
		matches[marker] = array( element,
			...
			"<elementattributesclosecontenttail" );
	}
	return stripped;
}
...
function insertStripItem( text ) {
	rnd = "{mUniqPrefix}-item-{mMarkerIndex}-" . self::MARKER_SUFFIX;
	mMarkerIndex++;
	mStripState->addGeneral( rnd, text );
	return rnd;
}
 ...
 * HTML. Only called for mOutputType == HTML.
 *
 * @private
 */
function internalParse( text, isMain = true, frame=false ) {
	...
	if ( !wfRunHooks( 'ParserBeforeInternalParse', array( &this, &text, &mStripState ) ) ) {
		wfProfileOut( __METHOD__ );
		return text ;
	}
	...
	text = Sanitizer::removeHTMLtags( text, array( &this, 'attributeStripCallback' ), false, array_keys( mTransparentTagHooks ) );
	wfRunHooks( 'InternalParseBeforeLinks', array( &this, &text, &mStripState ) );
	...
	text = doDoubleUnderscore( text );

	text = doHeadings( text );
	if ( mOptions->getUseDynamicDates() ) {
		df = DateFormatter::getInstance();
		text = df->reformat( mOptions->getDateFormat(), text );
	}
	text = replaceInternalLinks( text );
	text = doAllQuotes( text );
	...
	text = doMagicLinks( text );
	text = formatHeadings( text, origText, isMain );
...
function magicLinkCallback( m ) {
	if ( isset( m[1] ) && m[1] !== '' ) {
		...
		// Free external link
		return makeFreeExternalLink( m[0] );
...
function makeFreeExternalLink( url ) {
	...
	text = maybeMakeExternalImage( url );
	if ( text === false ) {
...
function replaceExternalLinks( text ) {
	...
	bits = preg_split( mExtLinkBracketedRegex, text, -1, PREG_SPLIT_DELIM_CAPTURE );
	s = array_shift( bits );
	...
	while ( i<count( bits ) ) {
		...
		img = maybeMakeExternalImage( text );
		if ( img !== false ) {
			text = img;
		}
		...
		if ( text == '' ) {
			// Autonumber
			langObj = getFunctionLang();
			text = '[' . langObj->formatNum( ++mAutonumber ) . ']';
			linktype = 'autonumber';
		} else {
		...
		s .= Linker::makeExternalLink( url, text, false, linktype,
			getExternalLinkAttribs( url ) ) . dtrail . trail;
...
function getExternalLinkAttribs( url = false ) {
	...
	if ( mOptions->getExternalLinkTarget() ) {
		attribs['target'] = mOptions->getExternalLinkTarget();
	}
	return attribs;
}
...
function maybeMakeExternalImage( url ) {
	...
	if ( mOptions->getAllowExternalImages()
	...
	if ( !text && mOptions->getEnableImageWhitelist()
...
function replaceInternalLinks( s ) {
	mLinkHolders->merge( replaceInternalLinks2( s ) );
	return s;
}
...
function replaceInternalLinks2( &s ) {
	...
	if ( wgContLang->hasVariants() ) {
		selflink = wgContLang->autoConvertToAllVariants( mTitle->getPrefixedText() );
	} else {
		selflink = array( mTitle->getPrefixedText() );
	}
	useSubpages = areSubpagesAllowed();
	wfProfileOut( __METHOD__.'-setup' );

	// Loop for each link
	for ( ; line !== false && line !== nullptr; a->next(), line = a->current() ) {
		...
		if ( useSubpages ) {
			link = maybeDoSubpageLink( m[1], text );
		} else {
			link = m[1];
		}
		...
		if ( might_be_img ) { // if this is actually an invalid link
			...
			if ( ns == NS_FILE && noforce ) { // but might be an image
				...
				while ( true ) {
					...
					a->next();
					next_line = a->current();
					if ( next_line === false || next_line === nullptr) {
						break;
					}
				...
				if ( !found ) {
					...
					holders->merge( replaceInternalLinks2( text ) );
					s .= "{prefix}[[link|text";
		...
		if ( noforce ) {
			...
			if ( iw && mOptions->getInterwikiMagic() && nottalk && wgContLang->getLanguageName( iw ) ) {
			...
			if ( ns == NS_FILE ) {
				...
				if ( !wfIsBadImage( nt->getDBkey(), mTitle ) ) {
					if ( wasblank ) {
		...
		if ( ns == NS_MEDIA ) {
			...
			list( file, nt ) = fetchFileAndTitle( nt, time, sha1 );
		...
		if ( iw == '' && nt->isAlwaysKnown() ) {
			mOutput->addLink( nt );
			s .= makeKnownLinkHolder( nt, text, array(), trail, prefix );
		} else {
...
function makeKnownLinkHolder( nt, text = '', query = array(), trail = '', prefix = '' ) {
	...
	link = Linker::linkKnown( nt, "prefixtextinside", array(), query );

	return armorLinks( link ) . trail;
}
...
function areSubpagesAllowed() {
	...
	return MWNamespace::hasSubpages( mTitle->getNamespace() );
}
...
function maybeDoSubpageLink( target, &text ) {
        return Linker::normalizeSubpageLink( mTitle, target, text );
}
...
function doBlockLevels( text, linestart ) {
	...
	foreach ( textLines as oLine ) {
		...
		if ( prefixLength && lastPrefix === prefix2 ) {
			...
			output .= nextItem( substr( prefix, -1 ) );
			paragraphStack = false;
			...
			commonPrefixLength = getCommon( prefix, lastPrefix );
			paragraphStack = false;

			// Close all the prefixes which aren't shared.
			while ( commonPrefixLength < lastPrefixLength ) {
				output .= closeList( lastPrefix[lastPrefixLength-1] );
				--lastPrefixLength;
			}

			// Continue the current prefix if appropriate.
			if ( prefixLength <= commonPrefixLength && commonPrefixLength > 0 ) {
				output .= nextItem( prefix[commonPrefixLength-1] );
			}

			// Open prefixes where appropriate.
			while ( prefixLength > commonPrefixLength ) {
				char = substr( prefix, commonPrefixLength, 1 );
				output .= openList( char );
		...
		if ( 0 == prefixLength ) {
			...
			closematch = preg_match(
				...
				'<td|<th|<\\/?div|<hr|<\\/pre|<\\/p|'.mUniqPrefix.'-pre|<\\/li|<\\/ul|<\\/ol|<\\/?center)/iS', t );
			if ( openmatch or closematch ) {
				...
				if ( ' ' == substr( t, 0, 1 ) and ( mLastSection === 'pre' || trim( t ) != '' ) ) {
					// pre
					if ( mLastSection !== 'pre' ) {
						paragraphStack = false;
	...
	while ( prefixLength ) {
		output .= closeList( prefix2[prefixLength-1] );
		--prefixLength;
	}
	if ( mLastSection != '' ) {
		output .= '</' . mLastSection . '>';
...
function getVariableValue( index, frame=false ) {
	 ...
	 */
	if ( wfRunHooks( 'ParserGetVariableValueVarCache', array( &this, &mVarCache ) ) ) {
		if ( isset( mVarCache[index] ) ) {
			return mVarCache[index];
		}
	}

	ts = wfTimestamp( TS_UNIX, mOptions->getTimestamp() );
	...
	switch ( index ) {
		...
		case 'localday2':
			...
			break;
		case 'pagename':
			value = wfEscapeWikiText( mTitle->getText() );
			break;
		case 'pagenamee':
			value = wfEscapeWikiText( mTitle->getPartialURL() );
			break;
		case 'fullpagename':
			value = wfEscapeWikiText( mTitle->getPrefixedText() );
			break;
		case 'fullpagenamee':
			value = wfEscapeWikiText( mTitle->getPrefixedURL() );
			break;
		case 'subpagename':
			value = wfEscapeWikiText( mTitle->getSubpageText() );
			break;
		case 'subpagenamee':
			value = wfEscapeWikiText( mTitle->getSubpageUrlForm() );
			break;
		case 'basepagename':
			value = wfEscapeWikiText( mTitle->getBaseText() );
			break;
		case 'basepagenamee':
			value = wfEscapeWikiText( wfUrlEncode( str_replace( ' ', '_', mTitle->getBaseText() ) ) );
			break;
		case 'talkpagename':
			if ( mTitle->canTalk() ) {
		...
		case 'revisionid':
			...
			mOutput->setFlag( 'vary-revision' );
			wfDebug( __METHOD__ . ": {{REVISIONID}} used, setting vary-revision...\n" );
			value = mRevisionId;
			break;
		case 'revisionday':
			// Let the edit saving system know we should parse the page
			// *after* a revision ID has been assigned. This is for nullptredits.
			mOutput->setFlag( 'vary-revision' );
			wfDebug( __METHOD__ . ": {{REVISIONDAY}} used, setting vary-revision...\n" );
			value = intval( substr( getRevisionTimestamp(), 6, 2 ) );
			break;
		case 'revisionday2':
			// Let the edit saving system know we should parse the page
			// *after* a revision ID has been assigned. This is for nullptredits.
			...
			value = substr( getRevisionTimestamp(), 6, 2 );
			break;
		case 'revisionmonth':
			// Let the edit saving system know we should parse the page
			// *after* a revision ID has been assigned. This is for nullptredits.
			...
			value = substr( getRevisionTimestamp(), 4, 2 );
			break;
		case 'revisionmonth1':
			// Let the edit saving system know we should parse the page
			// *after* a revision ID has been assigned. This is for nullptredits.
			...
			value = intval( substr( getRevisionTimestamp(), 4, 2 ) );
			break;
		case 'revisionyear':
			// Let the edit saving system know we should parse the page
			// *after* a revision ID has been assigned. This is for nullptredits.
			...
			value = substr( getRevisionTimestamp(), 0, 4 );
			break;
		...
		case 'revisionuser':
			...
			value = getRevisionUser();
			break;
		case 'namespace':
			value = str_replace( '_',' ',wgContLang->getNsText( mTitle->getNamespace() ) );
			break;
		case 'namespacee':
			value = wfUrlencode( wgContLang->getNsText( mTitle->getNamespace() ) );
			break;
		case 'talkspace':
			value = mTitle->canTalk() ? str_replace( '_',' ',mTitle->getTalkNsText() ) : '';
			break;
		case 'talkspacee':
			value = mTitle->canTalk() ? wfUrlencode( mTitle->getTalkNsText() ) : '';
			break;
		case 'subjectspace':
			value = mTitle->getSubjectNsText();
			break;
		case 'subjectspacee':
			value = ( wfUrlencode( mTitle->getSubjectNsText() ) );
			break;
		case 'currentdayname':
		...
		case 'currentweek':
			...
			// Nullable<int> to remove the padding
		...
		default:
			ret = nullptr;
			if ( wfRunHooks( 'ParserGetVariableValueSwitch', array( &this, &mVarCache, &index, &ret, &frame ) ) ) {
				return ret;
			} else {
				return nullptr;
			}
...
function initialiseVariables() {
	...
	mVariables = new MagicWordArray( variableIDs );
	mSubstWords = new MagicWordArray( substIDs );
	wfProfileOut( __METHOD__ );
}
...
function preprocessToDom( text, flags = 0 ) {
	dom = getPreprocessor()->preprocessToObj( text, flags );
	return dom;
}
 ...
 *  WIKI: only {{subst:}} templates
 *  PREPROCESS: templates but not extension tags
 *  HTML: all templates and extension tags
 *
...
function replaceVariables( text, frame = false, argsOnly = false ) {
	...
	if ( strlen( text ) < 1 || strlen( text ) > mOptions->getMaxIncludeSize() ) {
		return text;
	}
	wfProfileIn( __METHOD__ );

	if ( frame === false ) {
		frame = getPreprocessor()->newFrame();
	} elseif ( !( frame instanceof PPFrame ) ) {
		wfDebug( __METHOD__." called using plain parameters instead of a PPFrame instance. Creating custom frame.\n" );
		frame = getPreprocessor()->newCustomFrame( frame );
	}

	dom = preprocessToDom( text );
...
function limitationWarn( limitationType, current=null, max=null) {
	...
	mOutput->addWarning( warning );
	addTrackingCategory( "limitationType-category" );
}
...
function braceSubstitution( piece, frame ) {
	...
	// @todo FIXME: If piece['parts'] is nullptrthen the call to getLength() below won't work b/c this args isn't an object
	args = ( nullptr== piece['parts'] ) ? array() : piece['parts'];
	wfProfileOut( __METHOD__.'-setup' );
	...
	if ( !found ) {
		...
		substMatch = mSubstWords->matchStartAndRemove( part1 );
	...
	if ( !found && args->getLength() == 0 ) {
		id = mVariables->matchStartToEnd( part1 );
		if ( id !== false ) {
			text = getVariableValue( id, frame );
			if ( MagicWord::getCacheTTL( id ) > -1 ) {
				mOutput->updateCacheExpiry( MagicWord::getCacheTTL( id ) );
			}
			found = true;
		}
	}
	...
	if ( !found ) {
		...
		if ( colonPos !== false ) {
			...
			if ( isset( mFunctionSynonyms[1][function] ) ) {
				function = mFunctionSynonyms[1][function];
			} else {
				// Case insensitive functions
				function = wgContLang->lc( function );
				if ( isset( mFunctionSynonyms[0][function] ) ) {
			...
			if ( function ) {
				list( callback, flags ) = mFunctionHooks[function];
				...
				if ( !noparse ) {
					text = preprocessToDom( text, preprocessFlags );
					isChildObj = true;
				}
			}
		}
	...
	if ( !found ) {
		...
		if ( title ) {
			...
			limit = mOptions->getMaxTemplateDepth();
	...
	if ( !found && title ) {
		...
		if ( !title->isExternal() ) {
			if ( title->getNamespace() == NS_SPECIAL
				&& mOptions->getAllowSpecialInclusion()
				&& ot['html'] )
			{
				...
				list( text, title ) = getTemplateDom( title );
				if ( text !== false ) {
					found = true;
					isChildObj = true;
				}
			...
			if ( !found && ( ot['html'] || ot['pre'] ) ) {
				text = "[[:titleText]]";
				found = true;
			}
		} elseif ( title->isTrans() ) {
			// Interwiki transclusion
			if ( ot['html'] && !forceRawInterwiki ) {
				text = interwikiTransclude( title, 'render' );
				isHTML = true;
			} else {
				text = interwikiTransclude( title, 'raw' );
				// Preprocess it like a template
				text = preprocessToDom( text, self::PTD_FOR_INCLUSION );
				isChildObj = true;
			}
			found = true;
		}
	...
	if ( isChildObj ) {
		...
		if ( nowiki ) {
			...
			if ( isset( mTplExpandCache[titleText] ) ) {
				text = mTplExpandCache[titleText];
			} else {
				text = newFrame->expand( text );
				mTplExpandCache[titleText] = text;
			}
		} else {
			// Uncached expansion
			text = newFrame->expand( text );
	...
	if ( is_string( text ) && !incrementIncludeSize( 'post-expand', strlen( text ) ) ) {
		...
		text .= insertStripItem( '<!-- WARNING: template omitted, post-expand include size too large -->' );
		limitationWarn( 'post-expand-template-inclusion' );
	}
...
function getTemplateDom( title ) {
	...
	if ( isset( mTplRedirCache[titleText] ) ) {
		list( ns, dbk ) = mTplRedirCache[titleText];
		title = Title::makeTitle( ns, dbk );
		titleText = title->getPrefixedDBkey();
	}
	if ( isset( mTplDomCache[titleText] ) ) {
		return array( mTplDomCache[titleText], title );
	}

	// Cache miss, go to the database
	list( text, title ) = fetchTemplateAndTitle( title );

	if ( text === false ) {
		mTplDomCache[titleText] = false;
		return array( false, title );
	}

	dom = preprocessToDom( text, self::PTD_FOR_INCLUSION );
	mTplDomCache[ titleText ] = dom;

	if ( !title->equals( cacheTitle ) ) {
		mTplRedirCache[cacheTitle->getPrefixedDBkey()] =
...
function fetchTemplateAndTitle( title ) {
	templateCb = mOptions->getTemplateCallback(); // Defaults to Parser::statelessFetchTemplate()
	stuff = call_user_func( templateCb, title, this );
	text = stuff['text'];
	finalTitle = isset( stuff['finalTitle'] ) ? stuff['finalTitle'] : title;
	if ( isset( stuff['deps'] ) ) {
		foreach ( stuff['deps'] as dep ) {
			mOutput->addTemplate( dep['title'], dep['page_id'], dep['rev_id'] );
		}
	}
...
function fetchTemplate( title ) {
	rv = fetchTemplateAndTitle( title );
	return rv[0];
}
...
function fetchFile( title, time = false, sha1 = false ) {
	res = fetchFileAndTitle( title, time, sha1 );
	return res[0];
}
...
function fetchFileAndTitle( title, time = false, sha1 = false ) {
	...
	mOutput->addImage( title->getDBkey(), time, sha1 );
...
function interwikiTransclude( title, action ) {
	...
	return fetchScaryTemplateMaybeFromCache( url );
}
...
function argSubstitution( piece, frame ) {
	...
	if (  text === false && parts->getLength() > 0
	  && (
		ot['html']
		|| ot['pre']
		|| ( ot['wiki'] && frame->isTemplate() )
	  )
	) {
		// No match in frame, use the supplied default
		object = parts->item( 0 )->getChildren();
	}
	if ( !incrementIncludeSize( 'arg', strlen( text ) ) ) {
		error = '<!-- WARNING: argument omitted, expansion size too large -->';
		limitationWarn( 'post-expand-template-argument' );
	}
...
function extensionSubstitution( params, frame ) {
	...
	attrText = !isset( params['attr'] ) ? nullptr: frame->expand( params['attr'] );
	content = !isset( params['inner'] ) ? nullptr: frame->expand( params['inner'] );
	marker = "{mUniqPrefix}-name-" . sprintf( '%08X', mMarkerIndex++ ) . self::MARKER_SUFFIX;

	isFunctionTag = isset( mFunctionTagHooks[strtolower(name)] ) &&
		( ot['html'] || ot['pre'] );
	if ( isFunctionTag ) {
		markerType = 'none';
	} else {
	...
	if ( ot['html'] || isFunctionTag ) {
		...
		if ( isset( mTagHooks[name] ) ) {
			// Workaround for PHP bug 35229 and similar
			if ( !is_callable( mTagHooks[name] ) ) {
				throw new MWException( "Tag hook for name is not callable\n" );
			}
			output = call_user_func_array( mTagHooks[name],
				array( content, attributes, this, frame ) );
		} elseif ( isset( mFunctionTagHooks[name] ) ) {
			list( callback, flags ) = mFunctionTagHooks[name];
			if ( !is_callable( callback ) ) {
				throw new MWException( "Tag hook for name is not callable\n" );
			}
		...
		if ( content === nullptr) {
			...
			output = "<nameattrText>contentclose";
		}
	}
	...
	if ( markerType === 'none' ) {
		...
		mStripState->addNoWiki( marker, output );
	} elseif ( markerType === 'general' ) {
		mStripState->addGeneral( marker, output );
	} else {
...
function incrementIncludeSize( type, size ) {
	if ( mIncludeSizes[type] + size > mOptions->getMaxIncludeSize() ) {
		return false;
	} else {
		mIncludeSizes[type] += size;
		return true;
	}
}
 ...
 * Fills mDoubleUnderscores, returns the modified text
 */
function doDoubleUnderscore( text ) {
	...
	mDoubleUnderscores = mwa->matchAndRemove( text );

	if ( isset( mDoubleUnderscores['nogallery'] ) ) {
		mOutput->mNoGallery = true;
	}
	if ( isset( mDoubleUnderscores['notoc'] ) && !mForceTocPosition ) {
		mShowToc = false;
	}
	if ( isset( mDoubleUnderscores['hiddencat'] ) && mTitle->getNamespace() == NS_CATEGORY ) {
		addTrackingCategory( 'hidden-category-category' );
	}
	// (bug 8068) Allow control over whether robots index a page.
	#
	// @todo FIXME: Bug 14899: __INDEX__ always overrides __NOINDEX__ here!  This
	// is not desirable, the last one on the page should win.
	if ( isset( mDoubleUnderscores['noindex'] ) && mTitle->canUseNoindex() ) {
		mOutput->setIndexPolicy( 'noindex' );
		addTrackingCategory( 'noindex-category' );
	}
	if ( isset( mDoubleUnderscores['index'] ) && mTitle->canUseNoindex() ) {
		mOutput->setIndexPolicy( 'index' );
		addTrackingCategory( 'index-category' );
	}

	// Cache all double underscores in the database
	foreach ( mDoubleUnderscores as key => val ) {
		mOutput->setProperty( key, '' );
	}
...
protected function addTrackingCategory( msg ) {
	if ( mTitle->getNamespace() === NS_SPECIAL ) {
	...
	if ( containerCategory ) {
		mOutput->addCategory( containerCategory->getDBkey(), getDefaultSort() );
		return true;
	} else {
...
function formatHeadings( text, origText, isMain=true ) {
	...
	if ( isset( mDoubleUnderscores['noeditsection'] ) ) {
		showEditLink = 0;
	} else {
		showEditLink = mOptions->getEditSection();
	}
	if ( showEditLink ) {
		mOutput->setEditSectionTokens( true );
	}
	...
	enoughToc = mShowToc &&
		( ( numMatches >= 4 ) || mForceTocPosition );

	// Allow user to stipulate that a page should have a "new section"
	// link added via __NEWSECTIONLINK__
	if ( isset( mDoubleUnderscores['newsectionlink'] ) ) {
		mOutput->setNewSection( true );
	}

	// Allow user to remove the "new section"
	// link via __NONEWSECTIONLINK__
	if ( isset( mDoubleUnderscores['nonewsectionlink'] ) ) {
		mOutput->hideNewSection( true );
	}

	// if the string __FORCETOC__ (not case-sensitive) occurs in the HTML,
	// override above conditions and always show TOC above first header
	if ( isset( mDoubleUnderscores['forcetoc'] ) ) {
		mShowToc = true;
		enoughToc = true;
	}
	...
	toclevel = 0;
	prevtoclevel = 0;
	markerRegex = "{mUniqPrefix}-h-(\d+)-" . self::MARKER_SUFFIX;
	baseTitleText = mTitle->getPrefixedDBkey();
	oldType = mOutputType;
	setOutputType( WIKI );
	frame = getPreprocessor()->newFrame();
	root = preprocessToDom( origText );
	...
	foreach ( matches[3] as headline ) {
		...
		if ( preg_match("/^markerRegex/", headline, markerMatches ) ) {
			...
			list( titleText, sectionIndex ) = mHeadings[serial];
		...
		for( i = 1; i <= toclevel; i++ ) {
			if ( !empty( sublevelCount[i] ) ) {
				...
				numbering .= getFunctionLang()->formatNum( sublevelCount[i] );
				dot = 1;
			}
		}
		...
		safeHeadline = mStripState->unstripBoth( headline );

		// Remove link placeholders by the link text.
		//     <!--LINK number-->
		// turns into
		//     link text with suffix
		safeHeadline = replaceLinkHoldersText( safeHeadline );
		...
		if ( count( matches[3] ) > 1 && mOptions->getNumberHeadings() ) {
		...
		while ( node && !isTemplate ) {
			...
			byteOffset += mb_strlen( mStripState->unstripBoth(
		...
		tocraw[] = array(
			...
			'fromtitle' => titleText,
			'byteoffset' => ( isTemplate ? nullptr: byteOffset ),
			'anchor' => anchor,
		);
		...
		if ( sectionIndex !== false ) {
			...
			if ( isTemplate ) {
				...
				editlinkArgs = array( titleText, "T-sectionIndex"/*, nullptr*/ );
			} else {
				editlinkArgs = array( mTitle->getPrefixedText(), sectionIndex, headlineHint );
			}
	...
	if ( enoughToc ) {
		...
		toc = Linker::tocList( toc, mOptions->getUserLang() );
		mOutput->setTOCHTML( toc );
	}

	if ( isMain ) {
		mOutput->setSections( tocraw );
	}
...
public function preSaveTransform( text, Title title, User user, ParserOptions options, clearState = true ) {
	startParse( title, options, WIKI, clearState );
	setUser( user );
	...
	if( options->getPreSaveTransform() ) {
		text = pstPass2( text, user );
	}
	text = mStripState->unstripBoth( text );

	setUser( nullptr); #Reset
 ...
 *                  or nullptrto use default value
 * @return string
 */
function getUserSig( &user, nickname = false, fancySig = nullptr) {
	...
	nickname = nickname == nullptr? username : nickname;
	...
	if ( mb_strlen( nickname ) > wgMaxSigChars ) {
		...
		if ( validateSig( nickname ) !== false ) {
			// Validated; clean up (if needed) and return it
			return cleanSig( nickname, true );
		} else {
	...
	nickname = cleanSigInSig( nickname );
	...
	return wfMessage( msgName, userText, nickText )->inContentLanguage()->title( getTitle() )->text();
}
...
function cleanSig( text, parsing = false ) {
	if ( !parsing ) {
		...
		clearState();
		setTitle( wgTitle );
		setOutputType = PREPROCESS;
	}

	// Option to disable this feature
	if ( !mOptions->getCleanSignatures() ) {
		return text;
	}
 ...
 * @param title Title object or nullptrto use wgTitle
 * @return String
 */
public function transformMsg( text, options, title = nullptr) {
	...
	if ( !title ) {
		// It's not uncommon having a nullptrwgTitle in scripts. See r80898
		// Create a ghost title in such case
		title = Title::newFromText( 'Dwimmerlaik' );
	}
	text = preprocess( text, title, options );
...
public function setHook( tag, callback ) {
	...
	oldVal = isset( mTagHooks[tag] ) ? mTagHooks[tag] : nullptr;
	mTagHooks[tag] = callback;
	if ( !in_array( tag, mStripList ) ) {
		mStripList[] = tag;
	}
...
function setTransparentTagHook( tag, callback ) {
	...
	oldVal = isset( mTransparentTagHooks[tag] ) ? mTransparentTagHooks[tag] : nullptr;
...
public function setFunctionHook( id, callback, flags = 0 ) {
	...
	oldVal = isset( mFunctionHooks[id] ) ? mFunctionHooks[id][0] : nullptr;
	mFunctionHooks[id] = array( callback, flags );
...
function setFunctionTagHook( tag, callback, flags ) {
	...
	old = isset( mFunctionTagHooks[tag] ) ?
		mFunctionTagHooks[tag] : nullptr;
	mFunctionTagHooks[tag] = array( callback, flags );
...
function replaceLinkHolders( &text, options = 0 ) {
        return mLinkHolders->replace( text );
}
...
function replaceLinkHoldersText( text ) {
        return mLinkHolders->replaceText( text );
}
...
function renderImageGallery( text, params ) {
	...
	if ( isset( params['caption'] ) ) {
		...
		caption = replaceInternalLinks( caption );
	...
	foreach ( lines as line ) {
		...
		if ( isset( matches[3] ) ) {
			...
			matches[3] = recursiveTagParse( trim( matches[3] ) );
...
function getImageParams( handler ) {
	...
	if ( !isset( mImageParams[handlerClass]  ) ) {
		...
		if ( !internalParamMap ) {
			...
			foreach ( internalParamNames as type => names ) {
				foreach ( names as name ) {
					magicName = str_replace( '-', '_', "img_name" );
		...
		mImageParams[handlerClass] = paramMap;
		mImageParamsMagicArray[handlerClass] = new MagicWordArray( array_keys( paramMap ) );
	}
	return array( mImageParams[handlerClass], mImageParamsMagicArray[handlerClass] );
}
...
function makeImage( title, options, holders = false ) {
	...
	// Fetch and register the file (file title may be different via hooks)
	list( file, title ) = fetchFileAndTitle( title, time, sha1 );

	// Get parameter map
	handler = file ? file->getHandler() : false;

	list( paramMap, mwArray ) = getImageParams( handler );
			...
			if ( type === 'handler' && paramName === 'width' ) {
				...
				if ( type === 'handler' ) {
					...
					case 'alt':
						...
						validated = true;
						value = stripAltText( value, holders );
						break;
					case 'link':
						...
						if ( value === '' ) {
							...
							if ( preg_match( "/^(prots)chars+/", value, m ) ) {
								paramName = 'link-url';
								mOutput->addExternalLink( value );
								if ( mOptions->getExternalLinkTarget() ) {
									params[type]['link-target'] = mOptions->getExternalLinkTarget();
								}
								validated = true;
							}
						} else {
	...
	if ( imageIsFramed ) { // Framed image
		...
		if ( !isset( params['frame']['alt'] ) ) {
			...
			if ( caption !== '') {
				params['frame']['alt'] = stripAltText( caption, holders );
			} else {
		...
		params['frame']['title'] = stripAltText( caption, holders );
	}
	...
	ret = Linker::makeImageLink2( title, file, params['frame'], params['handler'],
		time, descQuery, mOptions->getThumbSize() );
...
protected function stripAltText( caption, holders ) {
	...
	if ( holders ) {
		...
		tooltip = replaceLinkHoldersText( caption );
	}

	// make sure there are no placeholders in thumbnail attributes
	// that are later expanded to html- so expand them now and
	// remove the tags
	tooltip = mStripState->unstripBoth( tooltip );
...
function disableCache() {
	...
	mOutput->setCacheTime( -1 ); // old style, for compatibility
...
function attributeStripCallback( &text, frame = false ) {
	text = replaceVariables( text, frame );
	text = mStripState->unstripBoth( text );
	return text;
}
 ...
 */
function getTags() {
        return array_merge( array_keys( mTransparentTagHooks ), array_keys( mTagHooks ) );
}
...
function replaceTransparentTags( text ) {
	matches = array();
	elements = array_keys( mTransparentTagHooks );
	text = self::extractTagsAndParams( elements, text, matches, mUniqPrefix );
	replacements = array();

	foreach ( matches as marker => data ) {
		list( element, content, params, tag ) = data;
		tagName = strtolower( element );
		if ( isset( mTransparentTagHooks[tagName] ) ) {
			output = call_user_func_array( mTransparentTagHooks[tagName], array( content, params, this ) );
		} else {
			output = tag;
		}
...
private function extractSections( text, section, mode, newText='' ) {
	...
	startParse( wgTitle, new ParserOptions, PLAIN, true );
	outText = '';
	frame = getPreprocessor()->newFrame();
	...
	// Preprocess the text
	root = preprocessToDom( text, flags );
	...
	if ( is_string( outText ) ) {
		...
		outText = rtrim( mStripState->unstripBoth( outText ) );
	}
...
/**
 * Get the revision object for mRevisionId
 *
 * @return Revision|nullptreither a Revision object or null
 */
protected function getRevisionObject() {
	if ( !is_null( mRevisionObject ) ) {
		return mRevisionObject;
	}
	if ( is_null( mRevisionId ) ) {
		return nullptr;
	}

	mRevisionObject = Revision::newFromId( mRevisionId );
	return mRevisionObject;
}
...
function getRevisionTimestamp() {
	if ( is_null( mRevisionTimestamp ) ) {
		wfProfileIn( __METHOD__ );

		global wgContLang;

		revObject = getRevisionObject();
...
function getRevisionUser() {
	if( is_null( mRevisionUser ) ) {
		revObject = getRevisionObject();

		// if this template is subst: the revision id will be blank,
		// so just use the current user's name
		if( revObject ) {
			mRevisionUser = revObject->getUserText();
		} elseif( ot['wiki'] || mOptions->getIsPreview() ) {
			mRevisionUser = getUser()->getName();
		}
	}
...
public function setDefaultSort( sort ) {
	mDefaultSort = sort;
	mOutput->setProperty( 'defaultsort', sort );
}
 ...
 * @return string
 */
function testSrvus( text, Title title, ParserOptions options, outputType = HTML ) {
        startParse( title, options, outputType, true );
...
function testPst( text, Title title, ParserOptions options ) {
	return preSaveTransform( text, title, options->getUser(), options );
}

function testPreprocess( text, Title title, ParserOptions options ) {
	return testSrvus( text, title, options, PREPROCESS );
}
...
function markerSkipCallback( s, callback ) {
	...
	out = '';
	while ( i < strlen( s ) ) {
		markerStart = strpos( s, mUniqPrefix, i );
...
function killMarkers( text ) {
        return mStripState->killMarkers( text );
}
...
function serializeHalfParsedText( text ) {
	...
	data = array(
		...
		'stripState' => mStripState->getSubState( text ),
		'linkHolders' => mLinkHolders->getSubArray( text )
	);
	wfProfileOut( __METHOD__ );
	return data;
}
...
function unserializeHalfParsedText( data ) {
	...
	texts = mStripState->merge( data['stripState'], texts );

	// Now renumber links
	texts = mLinkHolders->mergeForeign( data['linkHolders'], texts );
